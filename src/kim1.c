...
uint8_t read6502(uint16_t address) {
    ...
    if (address >= 0x1800 && address <= 0x1FFF) {  // KIM-1 ROM (HEX monitor): intercepts by Oscar Vermeulen
        ...
        if (address == 0x1E65) {                   // intercept GETCH (get char from serial).
            ...
            if (a != 13) addch((char)a);           // echo all chars but CR
            if (a == 0x40-'r') reset6502();        // fake RS button behavior on Ctrl-R press
    ...
    if (address >= 0x1700 && address <= 0x17FF) {  // KIM-1 6530 RIOT chips
        if (address == 0x1740) {                   // read key press
            uint8_t sv = (RIOT[0x42] >> 1) & 0xf;
            if (sv == 0) { return 0xff; 
            } else if (sv == 1) { return 0xff;
            } else if (sv == 2) { return 0xff;
            } else if (sv == 3) { return 0;        // always in serial mode
            } else return 0x80;
        }
...
void write6502(uint16_t address, uint8_t value) {
    ...
    return;                                        // skip non-writable memory
}

void handle_signal(int sig) {
    if (sig == SIGQUIT) reset6502();
}
